...
const Interchange = function() {
  Interchange.prototype.firmwares = firmwares;
...
Interchange.prototype.list_ports = function() {
  ...
  return Interchange.prototype.get_ports();
};
...
Interchange.prototype.install_firmware = async (firmware, options) => {
  ...
  try {
    // check the firmware
    const {fw, opts} = await Interchange.prototype.check_firmware(firmware, settings);
    ...
    const {hexpath, tmpdir} = await Interchange.prototype.download_firmware(fw, opts);

    /* istanbul ignore next */
    if (!process.env.TEST) {
      console.log('Flashing firmware to board');
    }
    const port = await Interchange.prototype.flash_firmware(hexpath, options)
      .then((serport) => {
        if (tmpdir) {
          Interchange.prototype.clean_temp_dir(tmpdir);
        }
        return serport;
      })
      .catch((err) => {
    ...
    if (!usingFirmata) {
      ...
      return Interchange.prototype.set_firmware_details(port, {...opts, ...fw})
        .then(() => { return true })
        .catch(err => { throw err });
    }
  } catch (e) {
